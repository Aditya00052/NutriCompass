...
st.set_page_config(page_title='Gemini Health App')          # must be the first streamlit command
...
# Render the header with the custom class
...
def get_gemini_response(input_prompt, image):
    ...
    # model = genai.GenerativeModel('gemini-pro-vision')    # gemini-pro-vision is deprecated

    # model = genai.GenerativeModel('gemini-1.5-flash')     # give so-so result           # try by using the img thing
    model = genai.GenerativeModel('gemini-1.5-pro')

    # response = model.generate_content([input_prompt, image[0]])    # when input_image_processing is run
...
def new_gemini_response(input_prompt, image):
    pass
...
# New: Uploading Image
# Create a layout with columns to add left margin
left_space, main_content = st.columns([3, 5])  # Adjust ratio as needed

# Place all elements within the main content column
with main_content:
    # Upload image widget
...
if submit:
    # image = input_image_processing(uploaded_file)
    image = Image.open(uploaded_file)
